...
[ASVspoof2019](https://datashare.ed.ac.uk/handle/10283/3336). В качестве модели
используется Light CNN ([arXiv:1904.05576](https://arxiv.org/abs/1904.05576))
с рецептом обучения из [arXiv:2103.11326](https://arxiv.org/abs/2103.11326),
...
официальному `grading.py`.** Результат получен фьюжном пяти наборов скоров:
четыре чекпоинта LFCC-модели и один модели на спектрограмме. Логи обучения:
[comet.com/a-ern/asvspoof-lcnn](https://www.comet.com/a-ern/asvspoof-lcnn).

## Решение

Скор системы считается как `logits[:, 1] - logits[:, 0]`. Реализация EER в
...
циклический повтор при паддинге, поскольку тишина в конце работает как ложная
подсказка. Модель: 9 свёрточных слоёв с Max-Feature-Map, 4 max-pooling,
dropout 0.75. Обучение: Adam, StepLR, Cross-Entropy с весами классов
`[1.0, 8.84]` под дисбаланс train, bfloat16-autocast.
...
окружении или `~/.comet.config` (без интернета укажите `writer.mode=offline`).
...
вторая задаёт eval-протокол, по которому проверяется полнота сабмита.
...
Все числа получены на полной eval-партиции той же реализацией EER, что у
грейдера.
